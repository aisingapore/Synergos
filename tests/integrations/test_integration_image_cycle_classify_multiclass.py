...
################################################################
# Integration Tests - Image Federated Cycle for MULTICLASS CLASSIFICATION #
################################################################

def test_integration_image_classify_cycle(
    driver,
    image_classify_cycle_multiclass_payloads
):

    project_payload = image_classify_cycle_multiclass_payloads['project']
    project_create_resp = driver.projects.create(**project_payload)
    assert project_create_resp['status'] in [200, 201]

    expt_payloads = image_classify_cycle_multiclass_payloads['experiment']
    ...
    run_payloads = image_classify_cycle_multiclass_payloads['run']
    for run_payload in run_payloads:
        run_create_resp = driver.runs.create(**run_payload)
        assert run_create_resp['status'] in [200, 201]

    participant_payloads = image_classify_cycle_multiclass_payloads['participant']
    ...
    registration_payloads = image_classify_cycle_multiclass_payloads['registration']
    ...
    validate_payloads = image_classify_cycle_multiclass_payloads['validation']
    ...
    predict_payloads = image_classify_cycle_multiclass_payloads['prediction']
