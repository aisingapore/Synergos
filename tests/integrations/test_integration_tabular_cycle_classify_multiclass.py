...
################################################################
# Integration Tests - Tabular Federated Cycle for CLASSIFICATION #
################################################################

def test_integration_tabular_classify_cycle(
    driver, 
    tabular_classify_cycle_payloads
):

    project_payload = tabular_classify_cycle_payloads['project']
    project_create_resp = driver.projects.create(**project_payload)
    assert project_create_resp['status'] in [200, 201]

    expt_payloads = tabular_classify_cycle_payloads['experiment']
    ...
    registration_payloads = tabular_classify_cycle_payloads['registration']
    ...
    validate_payloads = tabular_classify_cycle_payloads['validation']
